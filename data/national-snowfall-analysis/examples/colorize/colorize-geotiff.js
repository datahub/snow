...
program
    .usage('[options]')
    .option('-c, --colors <start>:<end>', 'Start and end of color ramp', parseRange)
    .option('-d, --values <start>:<end>', 'Start and end values (if empty, it guesses)', parseRange)
...
const [colorStart, colorEnd] = program.colors;
const [valueStart, valueEnd] = program.values
    .map((d) => {
        if (d === '') return null;
        return +d;
    });
...
const min = valueStart === null ? band.minimum : valueStart;
const max = valueEnd === null ? band.maximum : valueEnd;
